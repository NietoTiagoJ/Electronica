...
std::string getLastConnectedPort() {
    ...
    if (!pipe) {
        throw std::runtime_error("Error al ejecutar el comando para listar puertos.");
    }
    char buffer[128];
...
// Configuración de permisos en Linux
void configurarPermisosLinux(const std::string &port) {
#if defined(__linux__)
    std::string comando = "sudo chmod 666 " + port;
    int resultado = system(comando.c_str());
    if (resultado != 0) {
        throw std::runtime_error("Error al asignar permisos al puerto serial: " + port);
    }
#endif
}
...
int main() {
...
#if defined(__linux__)
        configurarPermisosLinux(serialPort);
#endif
    ...
    if (errorOpening != 1) {
        std::cerr << "Error al abrir el puerto serial: " << serialPort << std::endl;
        return errorOpening;
    }
